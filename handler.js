...
if (m.money) m.reply(+m.money + ' Pandicoins usados') 
}
break
}}} catch (e) {
console.error(e)
...
if (!m.fromMem && m.text.match(/(@593960070183|PandiBot|Botsito|Pandi|:v)/gi)) {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*ᴜɴ ɢʀᴜᴘᴏ 😸*\n *sɪɴ ʀᴇɢʟᴀ 😉*') :
