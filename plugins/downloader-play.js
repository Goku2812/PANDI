...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `*🛑♛•───★𝓜𝓪𝓻 𝔂 𝓒𝓱𝓪 𝓜𝓾𝓼𝓲𝓬★───•♛\n\n*[ 💡 ] ᴇᴊᴇᴍᴘʟᴏ:* _.ᴍʏᴄ ᴛᴇ ᴇɴᴄᴏɴᴛʀᴇ́ ᴇʟ ᴠᴇɢᴀ\n _${usedPrefix + command} https://youtu.be/L4lpAVro8Dk_\n\n♛•───★𝓜𝓪𝓻 𝔂 𝓒𝓱𝓪 𝓜𝓾𝓼𝓲𝓬★───•♛*`;
  try {
    const yt_play = await search(args.join(' '));
    let additionalText = '';
    if (command === 'myc') {
      additionalText = 'ᴀᴜᴅɪᴏ 📀';
    } else if (command === 'myc2') {
      additionalText = 'video 🎥';
    }
    const texto1 = `*⭐•───★𝓜𝓪𝓻 𝔂 𝓒𝓱𝓪 𝓜𝓾𝓼𝓲𝓬★───•⭐*\n*🍀ᴀ ᴘᴏʀ ᴇʟʟᴏ...${additionalText},*\n*🍀ᴄᴏɴ ɢᴜsᴛᴏ ᴛᴇ ʟᴏ ᴇɴᴠɪᴀᴍᴏs*\n*⭐•───★𝓜𝓪𝓻 𝔂 𝓒𝓱𝓪 𝓜𝓾𝓼𝓲𝓬★───•⭐*`.trim();
...
handler.help = ['myc', 'myc2'].map((v) => v + ' < busqueda >');
handler.tags = ['downloader'];
handler.command = /^(myc|myc2)$/i;
